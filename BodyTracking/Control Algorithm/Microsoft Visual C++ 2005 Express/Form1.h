	...
	for (unsigned int bodyIndex = 0; bodyIndex < bodyCount; bodyIndex++) {
		...
		if (SUCCEEDED(hr)) {
			...
			*/
			...
			int maxSpeedVal = 255;
			double maxBoxZ = 2.44;
			double maxDistZ = 3.66;

			double diffZ = maxDistZ - maxBoxZ;
			double errZ = posZ - maxBoxZ;
			double Kz = (maxSpeedVal - minSpeedVal) / diffZ;
			...
			if (errZ <= 0) {
				...
				speed = floor(Kz * errZ + minSpeedVal);
			}
			
			
			double maxBoxX = 0.61;
			double maxDistX = 2.745;
			double abs_posX = abs(posX);

			double diffX = maxDistX - maxBoxX;
			double Kx = maxSpeedVal / diffX;
