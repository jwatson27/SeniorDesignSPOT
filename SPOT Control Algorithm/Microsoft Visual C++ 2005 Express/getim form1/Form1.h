...
//-------------------------------------------------------BEGIN CUT AND PASTE BLOCK-----------------------------------------------------------------------------------
	...
	if(AttachedState == TRUE)
	{
		...
		{
			maxADCValueL = 0xFF; //ADCValueL; // TODO: Changed by Josiah Watson
		}
		if (ADCValueR > maxADCValueR)
		{
			maxADCValueR = 0xFF; //ADCValueR; // TODO: Changed by Josiah Watson
		}				
...
// TODO: Choose to only follow one skeleton
...
private: System::Void processBodies(const unsigned int &bodyCount, IBody **bodies)
{
	...
	for (unsigned int bodyIndex = 0; bodyIndex < bodyCount; bodyIndex++) {
		...
		///if (isTracked == false && bodyIndex == activeBodyIndex) { //bodyIndex == userBodyID) { // LOST TRACKING ON USER
			...
			{
				// GETIM: start
				transmitData(0x11, 0, 0, 0); // forward/forward, stop/stop, don't fire
				// GETIM: end
				///transmitData(0x11, 0, 0); // forward/forward, stop/stop
				// ORIGINAL: end
		...
		if (SUCCEEDED(hr)) {
			...
			// TODO: Change reinit distances to be more correct
			if (runInit) {
				// do initialization sequence
				userBodyID = bodyIndex;
				
				//if (posX > -1 && posX < 1) { // CENTERED: within 1 meter of center of kinect image
				//	if (posZ > 1 && posZ < 3) { // IN RANGE: within 1 to 3 meters away from kinect
				//		userBodyID = bodyIndex; // get skeleton ID of first skeleton processed
				//	}
				//}
				
				// convert RGB to HSV of area around spine_mid

				// get HSV avg

				runInit = false;
			}
			else if (bodyIndex == userBodyID) {
				
				// GETIM: start
				unsigned char inRange = 0;
				const double minRangeZ = 1.5; // meters
				const double maxRangeZ = 2; // meters
				const double minRangeX = -0.2; // meters (neg is right)
				const double maxRangeX = 0.2; // meters (pos is left)
				// GETIM: end
				// ORIGINAL: end
				...
				const double convRad2Deg = 180 / 3.14;

				//ACCValueX = (posZ * 100); // update distance
				//ACCValueY = (posX * 100); // update position
				//ACCValueZ = atan(angle) * convRad2Deg; // degrees

				updateGUI(posZ * 100, posX * 100, atan(angle)*convRad2Deg);

				const int BACKWARD = 0;
				const int FORWARD = 1;

				const int minSpeedValLeft = 10;    // previously 6
				const int minSpeedValRight = 15;   // previously 10
				const int maxSpeedVal = 255;

				const double maxBoxZ = 2.0;        // previously 2.44
				const double maxDistZ = 3.0;       // previously 3.66
				//const double maxBoxX = 0.61;
				const double thresholdAngle = 0.03; // 0.087; // tan(5-deg)
				//const double maxDistX = 2.745;
				const double maxAngle = 0.3; //0.577; // tan(30-deg)    was 25 deg 0.466

				const double maxDiffZ = maxDistZ - maxBoxZ;


				//REPLACED: const double maxDiffX = maxDistX - maxBoxX;		

				//const double maxDiffAngle = maxAngle - thresholdAngle;
				const double maxDiffAngle = maxAngle;



				const double Kz = maxSpeedVal / maxDiffZ; // steps per meter

				//REPLACED: const double Kx = maxSpeedVal / maxDiffX;
				const double Kx = maxSpeedVal / maxDiffAngle; // steps per angle
				...
				int turn;
				if (abs_angle <= thresholdAngle) {
					turn = 0;
				}
				else {
					double diff = (angle / abs_angle) * (abs_angle - thresholdAngle);
					turn = floor(Kx * diff * 0.5);
				}
				...
				// GETIM: start
				if (posZ > minRangeZ && posZ < maxRangeZ) { // if within range
					if (posX > minRangeX && posX < maxRangeX) {
						inRange = 1;
					}
				}
				// GETIM: end
				...
				const int maxIncrement = 1;

				int nextLSpd = lastLSpd + maxIncrement;
				int nextRSpd = lastRSpd + maxIncrement;
				...
				double slopeLeft = (double)(maxSpeedVal - minSpeedValLeft) / maxSpeedVal;
				double slopeRight = (double)(maxSpeedVal - minSpeedValRight) / maxSpeedVal;

				if (LSpd > 0) {
					LSpd = ceil(LSpd * slopeLeft) + minSpeedValLeft;
				}
				else {
					LSpd = 0;
				}
				if (RSpd > 0) {
					RSpd = ceil(RSpd * slopeRight) + minSpeedValRight;
				}
				else {
					RSpd = 0;
				}
				...
				if (AttachedState == TRUE) {
					// GETIM: start
					transmitData(directions, leftSpeed, rightSpeed, inRange);
					// GETIM: end
...
// GETIM: start
void transmitData(unsigned char dirs, unsigned char ls, unsigned char rs, unsigned char ir) {
// GETIM: end
