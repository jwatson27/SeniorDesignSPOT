...
//#include "Microchip/Include/Usb/usb_device.h"
...
//#include "Microchip/mTouch/mtouch.h"
#include "mtouch.h"

//#include "Microchip/BMA150 driver/BMA150.h"
#include "BMA150.h"

//#include "Microchip/OLED driver/oled.h"
#include "oled.h"

//#include "Microchip/Soft Start/soft_start.h"
...
// STARTING PARAMETERS FOR CCP MODULE
unsigned char pwmPeriodLeft = 0xFF;  // 340 us, 2.9 kHz
unsigned char pwmPeriodRight = 0xFF; // 340 us, 2.9 kHz
unsigned char pwmDutyCycleLeft = 0;  // 0 on startup
unsigned char pwmDutyCycleRight = 0; // 0 on startup
...
void ppsUnlock(void);
void ppsLock(void);
void setCCPperiod(unsigned char, unsigned char);
void setCCPspeed(unsigned char, unsigned char);
void initCCP(void);
void setupCCP(void);
...
//    #endif    
	...
	setupCCP(); // remap pins and set as outputs
...
{       
    ...
    {                                  
		switch(ReceivedDataBuffer[0])       //Look at the data the host sent, to see what kind of application specific command it sent.
        {
            ...
            case 0x00:  //both motors backward
                {                        
                    directions = 0x00;
                    leftSpeed = ReceivedDataBuffer[1];
                    rightSpeed = ReceivedDataBuffer[2];                     
                                                         
                    PORTBbits.RB5 = 0; // left backward
                    PORTBbits.RB4 = 0; // right backward                 
                    setCCPspeed(leftSpeed, rightSpeed); // set left and right speed                       
                    
                    // send values back to PC
                    ToSendDataBuffer[0] = 0x20;                    
                    ToSendDataBuffer[1] = directions;
                    ToSendDataBuffer[2] = leftSpeed;
                    ToSendDataBuffer[3] = rightSpeed;
                    
                    USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&ToSendDataBuffer[0],64);                                        
                }
                break;

            case 0x01:  //left backward right forward
                {
                    directions = 0x01;
                    leftSpeed = ReceivedDataBuffer[1];
                    rightSpeed = ReceivedDataBuffer[2];                     
                    ...
                    PORTBbits.RB5 = 0; // left backward
                    PORTBbits.RB4 = 1; // right backward                 
                    setCCPspeed(leftSpeed, rightSpeed); // set left and right speed                       
            ...
            case 0x10:  //left forward right backward
                {
                    directions = 0x10;
                    leftSpeed = ReceivedDataBuffer[1];
                    rightSpeed = ReceivedDataBuffer[2];                     
                                                         
                    PORTBbits.RB5 = 1; // left backward
                    PORTBbits.RB4 = 0; // right backward                 
                    setCCPspeed(leftSpeed, rightSpeed); // set left and right speed                       
...
void USBCBWakeFromSuspend(void)
{
    ...
    // ADDED BY JOSIAH WATSON 1-25-18    
	initCCP(); // set timers and pwm output
	setCCPperiod(pwmPeriodLeft, pwmPeriodRight); // set period
	setCCPspeed(pwmDutyCycleLeft, pwmDutyCycleRight);  // set duty cycle
}
...
// ADDED FUNCTIONS BY JOSIAH WATSON
...
void setCCPperiod(unsigned char periodL, unsigned char periodR) {
	pwmPeriodLeft = periodL;
	pwmPeriodRight = periodR;
...
void setCCPspeed(unsigned char dcL, unsigned char dcR) {
	pwmDutyCycleLeft = dcL;
	pwmDutyCycleRight = dcR;		
...
void setupCCP(void) {
    // Remap pins for CCP
	ppsUnlock();
		RPOR4 = 14; // remap CH1 (RB1) pin to CCP1 output        
		RPOR5 = 18; // remap CH2 (RB2) pin to CCP2 output	
	ppsLock();

	// Set pins as outputs
	TRISBbits.TRISB1 = 0; // CH1 (RB1) -- LEFT SPEED (CCP1)
	TRISBbits.TRISB2 = 0; // CH2 (RB2) -- RIGHT SPEED (CCP2)	
    TRISBbits.TRISB5 = 0; // A0 (RB5) -- LEFT DIRECTION
    TRISBbits.TRISB4 = 0; // A1 (RB4) -- RIGHT DIRECTION    
}



void initCCP(void) {
	TCLKCON = 0x01; // CCP1 uses T2, CCP2 uses T4
	T2CON = 0x06;   // T2 ON, prescaler of 1:16
	T4CON = 0x06;   // T4 ON, prescaler of 1:16    
	CCP1CON = 0x0C; // PWM single-output mode, output active high 
	CCP2CON = 0x0C; // PWM single-output mode, output active high		
}
...
//	DELAY TIMES
//        Delay10KTCYx(150); //150*10K =  500 ms
//        Delay1KTCYx(36);   // shortest delay where PWM works -- 36*1K =  12 ms
//        Delay10TCYx(96);   // longest delay where USB works  -- 96*10 =  0.32 ms 
